...
기존 Feature Group에 새로운 feature들을 추가하는 **통합 명령어**입니다.  
하나의 `add-features` 명령어로 **세 가지 다른 입력 방식**을 지원합니다.

## 기본 사용법

`add-features` 명령어는 다음 세 가지 방식 중 **하나**를 선택하여 사용할 수 있습니다:
...
```bash
# 기본 feature 추가
fs add-features my-fg -f "age:Integral:사용자 나이" -f "name:String:사용자 이름"

# 벡터 feature 추가  
fs add-features my-fg -f "embeddings:String:list:256:텍스트 임베딩"

# 집합 feature 추가
fs add-features my-fg -f "tags:String:set:사용자 태그"

# 미리보기
fs add-features my-fg -f "test_field:String" --dry-run
```

### 3. JSON 문자열 방식
```bash
# JSON 문자열로 feature 추가
fs add-features my-fg \
...
# 미리보기
fs add-features my-fg -j '{"FeatureName": "test", "FeatureType": "String"}' --dry-run
```

⚠️ **중요**: 한 번에 **하나의 입력 방식만** 사용할 수 있습니다. 여러 방식을 동시에 사용하면 오류가 발생합니다.
...
| 옵션 | 설명 | 기본값 | 사용 방식 |
|------|------|--------|---------|
| `FEATURE_GROUP_NAME` | 대상 Feature Group 이름 (필수) | - | 모든 방식 |
| `FEATURES_FILE` | JSON 파일 경로 (선택) | - | JSON 파일 방식 |
| `--feature`, `-f` | Feature 정의 문자열 (복수 가능) | - | CLI 플래그 방식 |
| `--json`, `-j` | JSON 형태의 feature 정의 (복수 가능) | - | JSON 문자열 방식 |
| `--dry-run` | 실제 추가 없이 미리보기만 실행 | False | 모든 방식 |
| `--wait/--no-wait` | 업데이트 완료까지 대기 여부 | True | 모든 방식 |

#### 입력 방식별 사용법:

**1. JSON 파일 방식:**
```bash
fs add-features <FEATURE_GROUP_NAME> <FEATURES_FILE> [옵션]
```

**2. CLI 플래그 방식:**
```bash
fs add-features <FEATURE_GROUP_NAME> -f "feature1" -f "feature2" [옵션]
```

**3. JSON 문자열 방식:**
```bash
fs add-features <FEATURE_GROUP_NAME> -j '{"FeatureName":"name1",...}' -j '{"FeatureName":"name2",...}' [옵션]
```
...
### 1. CLI 플래그 방식으로 Feature 추가

#### 기본 Feature 추가
```bash
# 단일 feature 추가
fs add-features my-feature-group -f "user_score:Fractional:사용자 점수"

# 여러 feature 동시 추가
fs add-features customer-profiles \
...
```bash
# 사용자 태그 집합
fs add-features user-profiles \
  -f "user_tags:String:set:사용자가 선택한 태그들"

# 관심 카테고리 집합
fs add-features customer-data \
  -f "interest_categories:String:set:관심 있는 상품 카테고리"

# 방문한 페이지 집합
fs add-features analytics \
...
fs add-features ecommerce-analytics \
...
### 2. JSON 문자열 방식으로 Feature 추가

#### 기본 JSON Feature 추가
```bash
# 단일 JSON feature
fs add-features my-feature-group \
  -j '{"FeatureName": "conversion_rate", "FeatureType": "Fractional", "Description": "전환율"}'

# 여러 JSON features 동시 추가
fs add-features ecommerce-features \
...
**실행 예시:**
```bash
$ fs add-features analytics-features \
...
# 다중 모달 특성 벡터
fs add-features multimodal-features \
  -j '{
...
for feature in "${BASE_FEATURES[@]}"; do
  echo "  추가 중: $feature"
  fs add-features "$FEATURE_GROUP" -f "$feature" --no-wait
done
...
| 상황 | 권장 방식 | 사용법 | 이유 |
|------|---------|-------|-----|
| **빠른 프로토타이핑** | CLI 플래그 방식 | `add-features fg -f "name:type"` | 즉석에서 테스트하기 쉽고 간단함 |
| **복잡한 벡터 Feature** | JSON 문자열 방식 | `add-features fg -j '{...}'` | 복잡한 설정을 정확하게 표현 가능 |
| **대량 Feature 추가** | JSON 파일 방식 | `add-features fg file.json` | 파일 관리, 버전 관리, 재사용 용이 |
| **스크립트 자동화** | CLI 플래그 방식 | `add-features fg -f "$var"` | 동적으로 생성하기 쉬움 |
| **문서화가 중요한 경우** | JSON 파일 방식 | `add-features fg schema.json` | 스키마 설명과 주석 포함 가능 |
| **CI/CD 파이프라인** | JSON 파일 방식 | `add-features fg features.json` | 버전 관리 및 리뷰 프로세스 적합 |
...
```bash
# 1단계: 프로토타이핑 (CLI 플래그)
fs add-features test-features -f "prototype_score:Fractional" --dry-run

# 2단계: 정교화 (JSON 문자열)
fs add-features test-features \
...
```bash
# 개발자: 빠른 실험
fs add-features dev-fg -f "experiment_metric:Fractional:실험용 지표"
...
```bash
# ❌ 비효율적: 하나씩 개별 추가
fs add-features fg -f "metric1:Fractional" 
fs add-features fg -f "metric2:Integral"
fs add-features fg -f "metric3:String"
...
for ((i=0; i<${#FEATURES[@]}; i+=BATCH_SIZE)); do
  ...
  fs add-features my-fg $(printf " -f %s" "${batch[@]}") --no-wait
...
```bash
# 개발 환경
fs add-features dev-features \
  -f "debug_score:Fractional:디버그용 점수" \
  -f "test_flag:String:테스트 플래그"

# 스테이징 환경  
fs add-features staging-features \
