...
Feature Group의 현재 스키마 정보 조회 또는 feature definition 템플릿 생성을 위한 **통합 명령어**입니다.  
하나의 `schema` 명령어로 **두 가지 다른 모드**를 지원합니다.

## 기본 사용법

`schema` 명령어는 다음 두 가지 모드 중 **하나**를 선택하여 사용할 수 있습니다:
...
⚠️ **중요**: 스키마 조회와 템플릿 생성을 동시에 할 수 없습니다. 하나의 모드만 선택해야 합니다.
...
| 옵션 | 설명 | 기본값 | 사용 모드 |
|------|------|--------|---------|
| `FEATURE_GROUP_NAME` | 조회할 Feature Group 이름 (선택) | - | 스키마 조회 모드 |
| `--output-format`, `-o` | 출력 형식 (table, json) | table | 스키마 조회 모드 |
| `--template` | Feature definition 템플릿 파일 생성 | False | 템플릿 생성 모드 |
| `--template-output` | 템플릿 출력 파일 경로 | feature_template.json | 템플릿 생성 모드 |

#### 모드별 사용법:

**1. 스키마 조회 모드:**
```bash
fs schema <FEATURE_GROUP_NAME> [옵션]
```
...
```bash
fs schema customer-profiles --output-format json
...
compare_schemas() {
    ...
    for fg in "${feature_groups[@]}"; do
        ...
        if fs schema "$fg" --output-format json > "$schema_file"; then
            echo "  ✅ $fg 스키마 수집 완료"
        else
            echo "  ❌ $fg 스키마 수집 실패"
            rm -rf "$temp_dir"
...
track_schema_evolution() {
    ...
    echo "💾 현재 스키마 저장 중..."
    if fs schema "$feature_group" --output-format json > "$current_schema"; then
...
validate_schema() {
    ...
    if ! fs schema "$feature_group" --output-format json > "$temp_schema"; then
...
generate_code_from_schema() {
    ...
    if ! fs schema "$feature_group" --output-format json > "$schema_file"; then
        echo "❌ 스키마 정보를 가져올 수 없습니다"
        return 1
    fi
...
diagnose_schema_issues() {
    ...
    if fs schema "$feature_group" --output-format json >/dev/null 2>&1; then
        echo "  ✅ 스키마 조회 성공"
        
        feature_count=$(fs schema "$feature_group" --output-format json | jq '.TotalFeatures')
        echo "  Feature 수: $feature_count"
    else
        echo "  ❌ 스키마 조회 실패"
        echo "  💡 상세 오류 확인:"
        fs schema "$feature_group" --output-format json 2>&1 | head -5
    fi
...
- `schema --template` - Feature definition 템플릿 생성 (통합됨)
